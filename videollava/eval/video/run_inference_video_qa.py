...
def run_inference(args):
    ...
    model.save_pretrained("/opt/llava-video-eran-2k-on-1378k/")
    torch.save(model.get_model().get_image_tower().state_dict(),
               "/opt/tpu-llava-video-eran-2k-on-1378k/llava/vision_tower_weights.pth")
    torch.save(model.get_model().mm_projector.state_dict(),
               "/opt/tpu-llava-video-eran-2k-on-1378k/llava/mm_projector_weights.pth")
    torch.save(model.get_model().get_video_tower().state_dict(),
               "/opt/tpu-llava-video-eran-2k-on-1378k/llava/video_tower_weights.pth")
    torch.save(model.lm_head.state_dict(), "/opt/tpu-llava-video-eran-2k-on-1378k/llava/lm_heads.pth")
    exit(0)
