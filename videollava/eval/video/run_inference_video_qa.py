...
def run_inference(args):
    ...
    model.save_pretrained("/opt/llava-video-eran-100k-1-epoch/")
    torch.save(model.get_model().get_image_tower().state_dict(),
               "/opt/tpu-llava-video-eran-100k-1-epoch/llava/vision_tower_weights.pth")
    torch.save(model.get_model().mm_projector.state_dict(),
               "/opt/tpu-llava-video-eran-100k-1-epoch/llava/mm_projector_weights.pth")
    torch.save(model.get_model().get_video_tower().state_dict(),
               "/opt/tpu-llava-video-eran-100k-1-epoch/llava/video_tower_weights.pth")
    torch.save(model.lm_head.state_dict(), "/opt/tpu-llava-video-eran-100k-1-epoch/llava/lm_heads.pth")
    exit(0)
