    ...
    for source in sources:
        for sentence in source:
            for DEFAULT_TOKEN in DEFAULT_X_TOKEN.values():
                X = DEFAULT_TOKEN[1:-1]  # <image> to image
    ...
    for conversation, target in zip(conversations, targets):
        ...
        for i, rou in enumerate(re_rounds):
            ...
            if len(parts) != 2:
                break
            parts[0] += sep
            round_len = len(tokenizer_X_token(rou, tokenizer, X_TOKEN_INDEX[X])) + len(tokenizer_X_token(conv.sep, tokenizer))
            instruction_len = len(tokenizer_X_token(parts[0], tokenizer, X_TOKEN_INDEX[X]))
            target[cur_len : cur_len + instruction_len] = IGNORE_INDEX
...
def preprocess(
    ...
    if conversation_lib.default_conversation.sep_style == conversation_lib.SeparatorStyle.LLAMA_2:
        return preprocess_llama_2(sources, tokenizer, X=X)
    if conversation_lib.default_conversation.version.startswith("v1"):
        return preprocess_v1(sources, tokenizer, X=X)
    if conversation_lib.default_conversation.version == "mpt":
        return preprocess_mpt(sources, tokenizer, X=X)
    # add end signal and concatenate together
    conversations = []
    for source in sources:
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        try:
            ...
            if 'image' in self.list_data_dict[i]:
                ...
                # 目前只支持一个sample一个modal
                if hasattr(self.data_args, 'image_processor') and hasattr(self.data_args, 'video_processor'):
                    # crop_size = self.data_args.image_processor.crop_size
                    crop_size = dict(height=224, width=224)
                    data_dict['image'] = torch.zeros(3, crop_size['height'], crop_size['width'])
                else:
                    if hasattr(self.data_args, 'image_processor'):
                        crop_size = self.data_args.image_processor.crop_size
                        data_dict['image'] = torch.zeros(3, crop_size['height'], crop_size['width'])
                    if hasattr(self.data_args, 'video_processor'):
                        data_dict['video'] = torch.zeros(3, 8, 224, 224)
            return data_dict
